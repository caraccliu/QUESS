...
void HashTable::removeAmbigSMers(uint64_t Tc, uint64_t Te, int seedNumber, uint64_t &peakMemory, uint64_t &currentMemory)
{
    ...
    uint64_t sGapsAboveTc = 0, sGapsAboveTe = 0, totalAmbigSMers = 0;
    uint64_t correctSGapPosition[9]= {0};
    for (uint64_t i = 0; i < size; ++i)
    {
     uint64_t maxCount[9]={0};
        if ((sMerTable[i].value != EMPTY) && (sMerTable[i].value != REMOVED) && (sMerTable[i].count < 255)) // count = 255 means ambiguous
        {
			...
			for (uint64_t k=0;k<sGapsAboveTc;k++){
				if (sGapTable[i][j].count>= maxCount[k]){
					for (uint64_t l=sGapsAboveTc+1;l<k;l--){
						correctSGapPosition[l] = maxCount[l-1];
						maxCount[l] = maxCount[l-1];	
					}				
				correctSGapPosition[k] = j;
                        	maxCount[k] = sGapTable[i][j].count;	
				}
			}
                   correctSGapPosition[sGapsAboveTc] = j;
		   maxCount[sGapsAboveTc]=sGapTable[i][j].count;
		   ++sGapsAboveTc;
	  ...
	  //if ((sGapsAboveTe >= 2) || (sGapsAboveTc == 0)) // ambiguous sMer (set counter to 255); remove its sGaps
            ...
            {               // store its score in sGapTable[i][0].count
                ...
                delete [] temp;*/
            	Element* temp = sGapTable[i];
		sGapTable[i] = new Element[sGapsAboveTc];
		for (uint64_t j = 0; j<sGapsAboveTc; j++){
                	sGapTable[i][j].count = min((int)(temp[correctSGapPosition[j]].count / max(maxErrSGapCount,(uint64_t)1)), 255); // score
                	sGapTable[i][j].value = temp[correctSGapPosition[j]].value;
		}
             	sMerTable[i].count = sGapsAboveTc;
        	delete [] temp; 
	}
	}
    }
...
int64_t HashTable::getCorrectSGap(uint64_t sMer, uint64_t sGap, uint64_t& correctSGap, uint64_t Tdiff, uint64_t* diff16Bits)
{
    ...
    for (uint64_t i = 0; i < sMerTable[place].count; i++){    
    // sMer not ambiguous; it has only one (correct) sGap at sGapTable[place][0] (because of "removeAmbigSMers")
    	if (sGap == sGapTable[place][i].value)  // sGap is already correct
        	return 0;
    }
   
    for (uint64_t i = 0; i < sMerTable[place].count; i++){ 
    //sGap is different from the correctSGap but must not be too different
    	if (getDiff(sGap, sGapTable[place][i].value, diff16Bits) < Tdiff)
    	{
        	correctSGap = sGapTable[place][i].value;        // return the correct sGap in "correctSGap"
        	return (int64_t)sGapTable[place][i].count;         // return also its score (from count)
	    }
    }
